					...
					columns : [
							...
							},
							{
								...
								mRender : function(data, type, row) {
									...
									if (data < 1) {
										return '<span style="color:red">Pas en stock!</span>';
									}
...
var $DetailsTable = $('#DetailsTable');
	...
	if($DetailsTable.length) {
		...
		var jsonUrl = window.contextRoot + '/json/data/admin/all/order';
		console.log(jsonUrl);
						    ...
						    bootbox.confirm({
						    	...
						    	callback: function (confirmed) {
							        if (confirmed) {
							            $.ajax({							            	
							            	type: 'GET',
							            	url: window.contextRoot + '/commande/'+checkbox.prop('value')+'/activation',
							        		timeout : 100000,
							        		success : function(data) {
							        			bootbox.alert(data);							        										        			
							        		},
